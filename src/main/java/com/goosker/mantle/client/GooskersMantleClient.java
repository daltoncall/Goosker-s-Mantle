...
public final class GooskersMantleClient implements ClientModInitializer {
    private static boolean suppressMantleUntilJumpRelease;
    ...
    private static void tickMantleInput(MinecraftClient client) {
        ...
        if (player == null || client.world == null) {
            suppressMantleUntilJumpRelease = false;
            return;
        }

        if (player.getAbilities().flying) {
            suppressMantleUntilJumpRelease = true;
            return;
        }
        if (suppressMantleUntilJumpRelease) {
